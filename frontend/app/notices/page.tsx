...
export default function NoticesPage() {
  ...
  // Reverse order so the most recently added notice shows first
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col justify-between">
      ...
      {/* Main notices content wrapper with top padding so it doesn't overlap fixed navbar */}
      <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 flex-grow">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl sm:text-5xl font-bold text-foreground mb-4">Latest Notices</h2>
          ...
          {loading ? (
            <div className="text-center py-12">
              <p className="text-lg text-foreground/70 italic">Loading notices...</p>
            </div>
          ) : displayNotices.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-lg text-foreground/70">No notices found in the database.</p>
            </div>
          ) : (
            <div className="flex flex-wrap justify-center gap-8">
              {displayNotices.map((notice) => {
                const isImagePath = notice.image && notice.image.startsWith('/');
                const isEmoji = notice.image && !notice.image.startsWith('/') && notice.image.length <= 4;
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {/* Notice content */}
                      <div className="p-6">
                        {/* Date placed cleanly at the top of the content */}
                        {notice.date && (
              ...
              <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative p-6 sm:p-8">
                ...
                {selectedNotice.image && (
                  <div className="h-64 sm:h-80 w-full relative mb-6 rounded-lg overflow-hidden bg-gray-100">
                    {selectedNotice.image.startsWith('/') ? (
                      <Image
